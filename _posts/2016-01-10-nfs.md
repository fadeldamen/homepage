...
Uma possível alternativa ao NFS seria o projeto **[GlusterFS](https://en.wikipedia.org/wiki/GlusterFS)** pois trata-se de um sistema cujo principal objetivo é a escalabilidade, sendo que para isso seus projetistas utilizaram conceitos da computação de alto desempenho, como a agregação. Este sistema pode executar sobre diversos sistemas operacionais, como GNU/Linux, FreeBSD, OpenSolaris e Mac OS X. Saiba mais sobre esta alternativa na página da **[documentação oficial do Gluterfs](http://www.gluster.org/community/documentation/index.php/GlusterDocumentation:About)**.
